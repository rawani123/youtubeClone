...
const Header = () => {
  ...
  const searchQueryHandler = (e) => {
    if (
      ...
      navigatte(`/searchResult/${searchQuery}`);
      setSearchQuery("")
    }
  };
          ...
          <div className="flex items-center">
            <div className="hidden md:flex">
              <div className="flex items-center justify-center h-10 w-10 rounded-full hover:bg-[#303030]/[0.6]">
                <RiVideoAddLine className="text-white text-3xl cursor-pointer "/>
              </div>
              <div className="flex items-center justify-center ml-2 h-10 w-10 rounded-full hover:bg-[#303030]/[0.6]">
                <FiBell className="text-white text-3xl cursor-pointer "/>
              </div>
              <div className="flex h-8 w-8 overflow-hidden rounded-full md:ml-4">
                <img src="https://api.multiavatar.com/zoe.svg" alt="profile Image" />
              </div>
            </div>
          </div>
      </div>
